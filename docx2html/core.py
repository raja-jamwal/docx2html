...
def is_highlighted(r):
    ...
    if rpr is None:
        return False
    highlighted = rpr.find('%shighlight' % w_namespace)
...
def get_t_tag_content(
    ...
    # underline)
    el_is_bold = not remove_bold and is_bold(parent)
    el_is_underlined = is_underlined(parent)
    el_is_italics = not remove_italics and is_italics(parent)
    el_is_highlighted = is_highlighted (parent)
 
    if el_is_highlighted:
        text = '<span style="background:yellow;">%s</span>' % text
    if el_is_underlined:
        text = '<u>%s</u>' % text
    if el_is_bold:
